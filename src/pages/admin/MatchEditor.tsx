    ...
    useEffect(() => {
        const fetchDefaults = async () => {
            try {
                ...
                if (settingsRes && settingsRes.defaultLineup) {
                    // Deep copy array to avoid ref issues
                    setLineup([...settingsRes.defaultLineup]);
                }
            } catch (err) {
    ...
    return (
        <div>
            <div className="page-header mb-8">
                <h1 className="page-title">Yeni Maç</h1>
            </div>
            ...
            <form onSubmit={handleSave} className="mb-12">
                ...
                <button type="submit" className="btn-primary" disabled={isSaving || !opponent}>
                    {isSaving ? 'Oluşturuluyor...' : 'Maçı Oluştur ve Skor Girişine İlerle →'}
                </button>
            </form>

            <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-8">
                <div>
                    <h2 className="text-2xl font-bold mb-4">Kadroyu Ayarla <span className="text-slate-400 text-sm font-normal">(Varsayılan ayarlardan çekildi)</span></h2>
                    <FormationGrid
                        lineup={lineup}
                        players={players}
                        interactive={true}
